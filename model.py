...
# 实例化 ConvBlock
conv_block = ConvBlock(in_channel=3, out_channel=64, strides=1)
...
with SummaryWriter("./network_visualization") as w:
    w.add_graph(conv_block, input_tensor)
...
class FastLeFF(nn.Module):
    ...
    def forward(self, x):
        # bs x hw x c
        bs, hw, c = x.size()
        hh = int(math.sqrt(hw))    # math.sqrt(hw)表示对hw开方
...
class InputProj(nn.Module):
    ...
    def forward(self, x):
        ...
        # self.proj(x) # B C H W
        # self.proj(x).flatten(2) # B C H*W
        # self.proj(x).flatten(2).transpose(1, 2) # B H*W C
        # self.proj(x).flatten(2).transpose(1, 2).contiguous() contigous()的作用是让tensor变成在内存中连续分布的形式
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size=input_size, embed_dim=96, depths=depths,
