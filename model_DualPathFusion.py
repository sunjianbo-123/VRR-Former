...
class GlobalFilter(nn.Module):
    ...
    def forward(self, x, spatial_size=None):
        ...
        # 创建一个全零的张量
        x_padded = torch.zeros((B, self.h, self.w, C), dtype=torch.float32, device=x.device)

        # 计算可以填充的位置范围
        start_h = torch.randint(0, self.h - a + 1, (1,)).item()
        start_w = torch.randint(0, self.w - b + 1, (1,)).item()

        # 将输入x填充到零张量的随机位置
        x_padded[:, start_h:start_h + a, start_w:start_w + b, :] = x



        x = torch.fft.rfft2(x_padded, dim=(1, 2), norm='ortho')
...
class FFTConvBlock(nn.Module):
    def __init__(self, dim, out_size, downsample, relu_slope, use_csff=False, use_FFT_PHASE=False, use_FFT_AMP=False):
        super(FFTConvBlock, self).__init__()
        self.downsample = downsample
        self.identity = nn.Conv2d(dim, out_size, 1, 1, 0)
        # 跨阶段特征融合
        self.use_csff = use_csff
        self.use_FFT_PHASE = use_FFT_PHASE
        self.use_FFT_AMP = use_FFT_AMP

        self.conv_1 = nn.Conv2d(dim, out_size, kernel_size=3, padding=1, bias=True)
...
class FFTLayer(nn.Module):
    def __init__(self, dim, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, depth_FFT=1, h=16, w=9,use_checkpoint=False):
        super().__init__()
        self.depth_FFT = depth_FFT
        self.use_checkpoint = use_checkpoint
        self.blocks = nn.ModuleList([])
        for _ in range(self.depth_FFT):
            self.blocks.append(FFTBlock(dim, mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path, act_layer=act_layer, norm_layer=norm_layer, h=h, w=w))
...
class FFTConvLayer(nn.Module):
    def __init__(self, dim, out_dim, downsample, relu_slope, use_csff=False, use_FFT_PHASE=False, use_FFT_AMP=False, depth_FFTConv=1, use_checkpoint=False):
        super().__init__()
        self.depth_FFTConv = depth_FFTConv
        self.use_checkpoint = use_checkpoint
        self.use_FFT_PHASE = use_FFT_PHASE
        self.use_FFT_AMP = use_FFT_AMP
        self.blocks = nn.ModuleList([])
        for _ in range(self.depth_FFTConv):
            self.blocks.append(FFTConvBlock(dim, out_dim, downsample, relu_slope, use_csff=False, use_FFT_PHASE=self.use_FFT_PHASE, use_FFT_AMP=self.use_FFT_AMP))
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans = 3, dd_in=3, patch_size = 4,
                 embed_dim=96, depths = [2, 2, 4, 8], depths_FFTConvblock=[2,2,2,2], num_heads = [2, 4, 6, 8],
                 ...
                 upsample_style="dual_upsample", downsample_style="patch_merging", **kwargs):
        super().__init__()
        ...
        self.encoderlayer_0 = TransformerLayer(dim=embed_dim,
        ...
        # self.FFT_encoderlayer_0 = FFTLayer(dim=embed_dim, mlp_ratio=4., drop=0., drop_path=0.,act_layer=nn.GELU,
        #                               norm_layer=norm_layer, depth_FFT=depths_FFTblock[0], h=1280//4, w=1280//8+1)
        self.FFTConv_encoderlayer_0 = FFTConvLayer(dim=embed_dim, out_dim=embed_dim, downsample=False, relu_slope=0.2,
                                                   depth_FFTConv=depths_FFTConvblock[0], use_csff=False, use_FFT_PHASE=False, use_FFT_AMP=True)
        ...
        self.encoderlayer_1 = TransformerLayer(dim=embed_dim * 2,
                                                ...
                                                token_projection=token_projection, token_mlp=token_mlp,attention_type=attention_type,
                                                shift_flag=shift_flag)
        # self.FFT_encoderlayer_1 = FFTLayer(dim=embed_dim*2, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU,
        #                               norm_layer=norm_layer, depth_FFT=depths_FFTblock[1], h=160, w=160//2+1)
        self.FFTConv_encoderlayer_1 = FFTConvLayer(dim=embed_dim * 2, out_dim=embed_dim * 2, downsample=False, relu_slope=0.2,
                                                   depth_FFTConv=depths_FFTConvblock[1], use_csff=False, use_FFT_PHASE=False, use_FFT_AMP=True)
        ...
        self.encoderlayer_2 = TransformerLayer(dim=embed_dim * 4,
                                                ...
                                                token_projection=token_projection, token_mlp=token_mlp,attention_type=attention_type,
                                                shift_flag=shift_flag)
        # self.FFT_encoderlayer_2 = FFTLayer(dim=embed_dim*4, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU,
        #                               norm_layer=norm_layer, depth_FFT=depths_FFTblock[2], h=80, w=80 // 2 + 1)
        self.FFTConv_encoderlayer_2 = FFTConvLayer(dim=embed_dim*4, out_dim=embed_dim*4, downsample=False, relu_slope=0.2,
                                                   depth_FFTConv=depths_FFTConvblock[2], use_csff=False, use_FFT_PHASE=False, use_FFT_AMP=True)
        ...
        # Bottleneck
        self.conv = TransformerLayer(dim=embed_dim * 8,
                                      ...
                                      token_projection=token_projection, token_mlp=token_mlp,attention_type=attention_type,
                                      shift_flag=shift_flag)
        # self.FFT_bottle = FFTLayer(dim=embed_dim*8, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU,
        #                               norm_layer=norm_layer, depth_FFT=depths_FFTblock[3], h=40, w=40//2 + 1)
        self.FFTConv_bottle = FFTConvLayer(dim=embed_dim*8, out_dim=embed_dim*8, downsample=False, relu_slope=0.2,
                                          depth_FFTConv=depths_FFTConvblock[3], use_csff=False, use_FFT_PHASE=False, use_FFT_AMP=False)
        ...
        self.decoderlayer_0 = TransformerLayer(dim=embed_dim * 4,
                                                output_dim=embed_dim * 4,
                                                input_resolution=(img_size // 16,
                                                                  img_size // 16),
                                                depth=depths[2],
                                                ...
                                                token_projection=token_projection, token_mlp=token_mlp,attention_type=attention_type,
                                                shift_flag=shift_flag)
        # self.FFT_decoderlayer_0 = FFTLayer(dim=embed_dim * 4, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU,
        #                               norm_layer=norm_layer, depth_FFT=depths_FFTblock[2], h=80,w=80 // 2 + 1)
        self.FFTConv_decoderlayer_0 = FFTConvLayer(dim=embed_dim*4, out_dim=embed_dim*4, downsample=False, relu_slope=0.2,
                                          depth_FFTConv=depths_FFTConvblock[2], use_csff=False, use_FFT_PHASE=True, use_FFT_AMP=False)
        ...
        self.decoderlayer_1 = TransformerLayer(dim=embed_dim * 2,
                                                output_dim=embed_dim * 2,
                                                input_resolution=(img_size // 8,
                                                                  img_size // 8),
                                                depth=depths[1],
                                                ...
                                                token_projection=token_projection, token_mlp=token_mlp,attention_type=attention_type,
                                                shift_flag=shift_flag)
        # self.FFT_decoderlayer_1 = FFTLayer(dim=embed_dim * 2, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU,
        #                               norm_layer=norm_layer, depth_FFT=depths_FFTblock[1], h=160,
        #                               w=160// 2 + 1)
        self.FFTConv_decoderlayer_1 = FFTConvLayer(dim=embed_dim*2, out_dim=embed_dim*2, downsample=False, relu_slope=0.2,
                                          depth_FFTConv=depths_FFTConvblock[1], use_csff=False, use_FFT_PHASE=True, use_FFT_AMP=False)
        ...
        self.decoderlayer_2 = TransformerLayer(dim=embed_dim,
                                                output_dim=embed_dim,
                                                input_resolution=(img_size // 4,
                                                                  img_size // 4),
                                                depth=depths[0],
                                                ...
                                                token_projection=token_projection, token_mlp=token_mlp,attention_type=attention_type,
                                                shift_flag=shift_flag)
        # self.FFT_decoderlayer_2 = FFTLayer(dim=embed_dim , mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU,
        #                               norm_layer=norm_layer, depth_FFT=depths_FFTblock[0], h=320,
        #                               w=320 // 2 + 1)
        self.FFTConv_decoderlayer_2 = FFTConvLayer(dim=embed_dim, out_dim=embed_dim, downsample=False, relu_slope=0.2,
                                          depth_FFTConv=depths_FFTConvblock[0], use_csff=False, use_FFT_PHASE=True, use_FFT_AMP=False)
    ...
    def forward(self, x):
        ...
        # stage 0
        transformer_encoder0 = self.encoderlayer_0(y)                                        # 1, 4096, 96
        # fft_encoder0 = self.FFT_encoderlayer_0(y)                                          # 1, 4096, 96
        fftconv_encoder0 = self.FFTConv_encoderlayer_0(y)
        fusion_encoder0 = transformer_encoder0 + fftconv_encoder0
        pool0 = self.dowsample_0(fusion_encoder0)                                          # 1, 4096, 96 -> 1, 1024, 192


        # stage 1
        transformer_encoder1 = self.encoderlayer_1(pool0)                               # 1, 1024, 192
        # fft_encoder1 = self.FFT_encoderlayer_1(pool0)                                   # 1, 1024, 192
        fftconv_encoder1 = self.FFTConv_encoderlayer_1(pool0)
        fusion_encoder1 = transformer_encoder1 + fftconv_encoder1
        pool1 = self.dowsample_1(fusion_encoder1)                                       # 1, 1024, 192 -> 1, 256, 384

        # stage 2
        transformer_encoder2 = self.encoderlayer_2(pool1)                              # 1, 256, 384
        # fft_encoder2 = self.FFT_encoderlayer_2(pool1)                                  # 1, 256, 384
        fftconv_encoder2 = self.FFTConv_encoderlayer_2(pool1)
        fusion_encoder2 = transformer_encoder2 + fftconv_encoder2
        pool2 = self.dowsample_2(fusion_encoder2)                                      # 1, 256, 384 -> 1, 64, 768


        # Bottleneck
        conv3 = self.conv(pool2)                                                       # 64, 768
        # fft_bottle = self.FFT_bottle(pool2)                                          # 64, 768
        fftconv_bottle = self.FFTConv_bottle(pool2)
        fusion_bottle = conv3 + fftconv_bottle


        # Decoder
        # stage 0
        up0 = self.up_0(fusion_bottle)                                              # 64, 768 -> 256, 384
        deconv0 = torch.cat([up0, fusion_encoder2], -1)                      # 256, 768
        deconv0_back_dim = self.concat_back_dim_0(deconv0)                          # 256, 768 -> 256, 384
        transformer_decoder0 = self.decoderlayer_0(deconv0_back_dim)                # 256, 384
        # fft_decoder0 = self.FFT_decoderlayer_0(deconv0_back_dim)                    # 256, 384
        fftconv_decoder0 = self.FFTConv_decoderlayer_0(deconv0_back_dim)
        fusion_decoder0 = transformer_decoder0 + fftconv_decoder0                      # 256, 384

        # stage 1
        up1 = self.up_1(fusion_decoder0)                                            # 256, 384 -> 1024, 192
        deconv1 = torch.cat([up1, fusion_encoder1], -1)                      # 1024, 384
        deconv1_back_dim = self.concat_back_dim_1(deconv1)                          # 1024, 384 -> 1024, 192
        transformer_decoder1 = self.decoderlayer_1(deconv1_back_dim)                # 1024, 192
        # fft_decoder1 = self.FFT_decoderlayer_1(deconv1_back_dim)                  # 1024, 192
        fftconv_decoder1 = self.FFTConv_decoderlayer_1(deconv1_back_dim)
        fusion_decoder1 = transformer_decoder1 + fftconv_decoder1                   # 1024, 192


        up2 = self.up_2(fusion_decoder1)                                               # 1024, 192 -> 4096, 96
        deconv2 = torch.cat([up2, fusion_encoder0], -1)                         # 4096, 192
        deconv2_back_dim = self.concat_back_dim_2(deconv2)                             # 4096, 192 -> 4096, 96
        deconv2 = self.decoderlayer_2(deconv2_back_dim)                                # 4096, 96
        # fft_decoder2 = self.FFT_decoderlayer_2(deconv2_back_dim)                     # 4096, 96
        fftconv_decoder2 = self.FFTConv_decoderlayer_2(deconv2_back_dim)
        fusion_decoder2 = deconv2 + fftconv_decoder2

        # Output Projection
        y = self.up_3(fusion_decoder2)                                               # 1, 64*64, 96    -> 1, 128*128, 48
...
if __name__ == "__main__":
    ...
    arch = Uformer
    depths = [2, 2, 2, 2]
    depths_FFTConv = [2, 4, 4, 2]
    model_restoration = Uformer(img_size=input_size, embed_dim=96, patch_size=4, depths=depths, depths_FFTConvblock=depths_FFTConv,
                                ...
                                shift_flag=True, upsample_style="dual_upsample", downsample_style="conv_downsample", final_upsample="dual_upsample")
