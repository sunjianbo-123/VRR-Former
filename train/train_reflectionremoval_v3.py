    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        if (i + 1) % eval_now == 0 and i > 0:
            with torch.no_grad():
                ...
                print(f"[{epoch}/{i}  PSNR:{psnr_val_rgb:.4f}\tSSIM:{ssim_val_rgb:.4f}]  ----  [Best_PSNR:{best_psnr:.4f}  {best_epoch_psnr}/{best_iter_psnr}\tBest_SSIM:{best_ssim:.4f}  {best_epoch_ssim}/{best_iter_ssim}]")
                with open(logname, 'a') as f:
                    f.write(
                     f"[{epoch}/{i}  PSNR:{psnr_val_rgb:.4f}\tSSIM:{ssim_val_rgb:.4f}]  ----  [Best_PSNR:{best_psnr:.4f}  {best_epoch_psnr}/{best_iter_psnr}\tBest_SSIM:{best_ssim:.4f}  {best_epoch_ssim}/{best_iter_ssim}" + '\n')
