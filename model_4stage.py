...
class InputProj(nn.Module):
    ...
    def __init__(self, in_channel=3, embed_dim=96, kernel_size=3, norm_layer=nn.LayerNorm,
                 act_layer=nn.GELU):
        super().__init__()

        self.proj = nn.Sequential(
            nn.Conv2d(in_channel, embed_dim, kernel_size=3, stride=1, padding=kernel_size // 2),
            act_layer(),
        )
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans = 3, dd_in=3, patch_size = 4,
                 embed_dim=96, depths = [2, 2, 4, 8], num_heads = [2, 4, 6, 8],
        ...
        # Input/Output
        self.input_proj = InputProj(in_channel=dd_in, embed_dim=embed_dim, act_layer=nn.GELU)

        # 256*256,32 -> 1,3,256,256
        self.output_proj = OutputProj(in_channel=embed_dim, out_channel=in_chans, kernel_size=3, stride=1)

        # Encoder        256,256,32 -> 256,256,32
        ...
        # Downsample    256,256,32 -> 128,128,64
        if downsample_style == "patch_merging":
            self.dowsample_0 = PatchMerging(input_resolution=(img_size, img_size), dim=embed_dim)
        elif downsample_style == "conv_downsample":
            self.dowsample_0 = Downsample(embed_dim, embed_dim * 2)

        # Encoder        128,128,64 -> 128,128,64
        self.encoderlayer_1 = BasicUformerLayer(dim=embed_dim * 2,
                                                ...
                                                token_projection=token_projection, token_mlp=token_mlp,attention_type=attention_type,
                                                shift_flag=shift_flag)
        # Downsample    128,128,64 -> 64,64,128
        if downsample_style == "patch_merging":
            self.dowsample_1 = PatchMerging(input_resolution=(img_size // 2, img_size // 2),dim=embed_dim * 2)
        elif downsample_style == "conv_downsample":
            self.dowsample_1 = Downsample(embed_dim * 2, embed_dim * 4)

        # Encoder        64,64,128 -> 64,64,128
        self.encoderlayer_2 = BasicUformerLayer(dim=embed_dim * 4,
                                                ...
                                                token_projection=token_projection, token_mlp=token_mlp,attention_type=attention_type,
                                                shift_flag=shift_flag)
        # Downsample    64,64,128 -> 32,32,256
        if downsample_style == "patch_merging":
            self.dowsample_2 = PatchMerging(input_resolution=(img_size // 4,img_size // 4),dim=embed_dim * 4)
        elif downsample_style == "conv_downsample":
            self.dowsample_2 = Downsample(embed_dim * 4, embed_dim * 8)

        # Encoder       32,32,256 -> 32,32,256
        self.encoderlayer_3 = BasicUformerLayer(dim=embed_dim * 8,
                                                ...
                                                shift_flag=shift_flag)
        # Downsample    32,32,256 -> 16,16,512
        if downsample_style == "patch_merging":
            self.dowsample_3 = PatchMerging(input_resolution=(img_size // 8, img_size // 8), dim=embed_dim * 8)
        elif downsample_style == "conv_downsample":
        ...
        # Bottleneck   16,16,512 -> 16,16,512
        self.conv = BasicUformerLayer(dim=embed_dim * 16,
                                      ...
                                      norm_layer=norm_layer,
                                      use_checkpoint=use_checkpoint,
                                      token_projection=token_projection, token_mlp=token_mlp,attention_type=attention_type,shift_flag=shift_flag)

        # up0  16x16, 512 --> 32x32, 256
        if self.upsample_style == "dual_upsample":
            self.up_0 = UpSample_Dual(input_resolution=(img_size // 16, img_size // 16),
                                  in_channels=embed_dim * 16, scale_factor=2)
        elif self.upsample_style == "transpose_conv_upsample":
            self.up_0 = Transpose_Upsample(embed_dim * 16, embed_dim * 8)

        # up0+conv3 32x32, 512 --> 32x32, 256
        self.concat_back_dim_0 = nn.Linear(embed_dim * 16, embed_dim * 8)
        ...
        # up1 32x32, 256 --> 64x64, 128
        if self.upsample_style == "dual_upsample":
            self.up_1 = UpSample_Dual(input_resolution=(img_size // 8, img_size // 8),
                                  in_channels=embed_dim * 8, scale_factor=2)
        elif self.upsample_style == "transpose_conv_upsample":
            self.up_1 = Transpose_Upsample(embed_dim * 8, embed_dim * 4)

        # up1+conv2  64x64, 256 --> 64x64, 128
        self.concat_back_dim_1 = nn.Linear(embed_dim * 8, embed_dim * 4)
        # 64x64, 128 --> 64x64, 128
        ...
        # 64x64, 128 -> 128*128, 64
        if self.upsample_style == "dual_upsample":
            self.up_2 = UpSample_Dual(input_resolution=(img_size // 4, img_size // 4),
                                  in_channels=embed_dim * 4, scale_factor=2)
        elif self.upsample_style == "transpose_conv_upsample":
            self.up_2 = Transpose_Upsample(embed_dim * 4, embed_dim * 2)

        # 128*128, 128 -> 128*128, 64
        self.concat_back_dim_2 = nn.Linear(embed_dim * 4, embed_dim *2)
        ...
        if self.upsample_style == "dual_upsample":
            self.up_3 = UpSample_Dual(input_resolution=(img_size // 2, img_size // 2),
                                      in_channels=embed_dim * 2, scale_factor=2)
        elif self.upsample_style == "transpose_conv_upsample":
            self.up_3 = Transpose_Upsample(embed_dim * 2, embed_dim)

        # 256*256, 64 -> 256*256, 32
        self.concat_back_dim_3 = nn.Linear(embed_dim * 2, embed_dim)
        # 256*256, 32
    ...
    def forward(self, x, mask=None):
        # Input Projection
        y = self.input_proj(x)                                      # 1, 3, 256, 256 -> 1, 65536, 32
        y = self.pos_drop(y)
        # Encoder
        conv0 = self.encoderlayer_0(y, mask=mask)                   # 1, 256*256, 32
        pool0 = self.dowsample_0(conv0)                             # 1, 256*256, 32 -> 1, 128*128, 64
        conv1 = self.encoderlayer_1(pool0, mask=mask)               # 1, 128*128, 64
        pool1 = self.dowsample_1(conv1)                             # 1, 128*128, 64 -> 1, 64*64, 128
        conv2 = self.encoderlayer_2(pool1, mask=mask)               # 1, 64*64, 128
        pool2 = self.dowsample_2(conv2)                             # 1, 64*64, 128 -> 1, 32*32, 256
        conv3 = self.encoderlayer_3(pool2, mask=mask)               # 1, 1024, 256
        pool3 = self.dowsample_3(conv3)                             # 1, 1024, 256 -> 1, 256, 512

        # Bottleneck
        conv4 = self.conv(pool3, mask=mask)                         # 1, 256, 512


        # Decoder
        up0 = self.up_0(conv4)                                      # 16*16, 512 -> 32*32, 256
        deconv0 = torch.cat([up0, conv3], -1)                # 32*32,256  -> 32*32, 512
        deconv0_back_dim = self.concat_back_dim_0(deconv0)          # 32*32, 512 -> 32*32, 256
        deconv0 = self.decoderlayer_0(deconv0_back_dim, mask=mask)  # 32*32, 256

        up1 = self.up_1(deconv0)                                    # 32*32, 256 -> 64*64, 128
        deconv1 = torch.cat([up1, conv2], -1)                # 64*64,128  -> 64*64，256
        deconv1_back_dim = self.concat_back_dim_1(deconv1)          # 64*64, 256 -> 64*64, 128
        deconv1 = self.decoderlayer_1(deconv1_back_dim, mask=mask)  # 64*64, 128

        up2 = self.up_2(deconv1)                                    # 64*64, 128 -> 128*128，64
        deconv2 = torch.cat([up2, conv1], -1)                # 128*128,64 -> 128*128,128
        deconv2_back_dim = self.concat_back_dim_2(deconv2)          # 128*128,128 -> 128*128,64
        deconv2 = self.decoderlayer_2(deconv2_back_dim, mask=mask)  # 128*128,64

        up3 = self.up_3(deconv2)                                    # 128*128,64 -> 256*256，32
        deconv3 = torch.cat([up3, conv0], -1)                # 256*256,32 -> 256*256, 64
        deconv3_back_dim = self.concat_back_dim_3(deconv3)          # 256*256,64 -> 256*256, 32
        deconv3 = self.decoderlayer_3(deconv3_back_dim, mask=mask)  # 256*256,32

        # Output Projection
        y = self.output_proj(deconv3)                               # 1,256*256,32  -> 1,3,256,256
...
if __name__ == "__main__":
    arch = Uformer
    model_restoration = Uformer(img_size=256,in_chans=3,dd_in=3,patch_size=4,embed_dim=32,depths = [2,2,4,8,2,8,4,2,2], num_heads = [2,2,4,8,16,8,4,2,2],
                                    win_size=8,mlp_ratio=4.,token_projection="linear",token_mlp="g_ffn",attention_type="HiLo_attention",
                                    shift_flag=True, modulator=False, cross_modulator=False,final_upsample ="dual_upsample",
                                    upsample_style="dual_upsample", downsample_style="conv_downsample")
